...
The methodological distinction between these two approaches is set out in \Cref{tab:manual-automated-testing-approaches}.
...
As a simplified illustration, the same test intent can be expressed in either a manual or an automated form, as contrasted in \Cref{tab:manual-automation-representations}. This example is illustrative rather than project-result evidence.
...
Several test design considerations were adopted as methodological guidance for this study, drawing on ISTQB test-case design guidance \autocite{istqb_glossary_test_case_2026} and systematic testing criteria \autocite{ammann_offutt_testing_2016}; these author-synthesized considerations are collected in \Cref{tab:automated-test-design-considerations}.
...
This literature suggests that a framework is more than a folder containing scripts. A useful automation framework provides structure for defining tests, executing them, adapting them to the system under test, managing data and configuration, and reviewing results. These framework concepts are consolidated in \Cref{tab:literature-based-framework-concepts} as the theoretical baseline for Chapter~3; the "Design implication for later chapters" column represents author-derived inference from the cited literature rather than a direct quotation from any single source.
