...
| T-093 | Phase 2 convert chapters 1–6 from approved Markdown to LaTeX, one chapter per approval gate (compile + content-equivalence + visual review before next chapter). | IN_PROGRESS | P0 | Codex | Chapter 1 (`01-introduction.tex`) converted, compiled, reviewed, and **user-approved 2026-06-12**. **Next: Chapter 2** (`chapter-2-literature-review.md` → `02-literature-review.tex`), one chapter per gate. Bibliography warnings remain expected until T-096. |
| T-094 | Phase 2 convert front matter: cover, approval/declaration, acknowledgements, abstract, abbreviations, definitions, author contribution — using the author/supervisor metadata from `README.md`, not the sample's personal data. | BLOCKED | P0 | Codex | Front-matter **structure** done and user-approved 2026-06-12 (title page + copyright + degree statement, approval page in VNUK wording, TOC with dotted leaders + List of Figures/Tables, section renames). **Remaining = CONTENT conversion** of stub pages: `acknowledgements.tex`, `abstract.tex`, `definitions.tex`, `abbreviations.tex`, `author-contribution.tex` (still `\migrationpending` placeholders). Output: `docs/thesis-latex/frontmatter/*.tex` |
| T-095 | Phase 2 convert appendices A–E to LaTeX. | BLOCKED | P1 | Codex | `docs/thesis-latex/appendices/*.tex` |
| T-096 | Bibliography migration: port `references.bib`, set `biblatex style=apa` (APA 7 confirmed by user 2026-06-12), verify every `[@key]`/`\cite{}` resolves and no unused/undefined entries remain. | IN_PROGRESS | P0 | Claude/Codex | `references.bib` ported into `docs/thesis-latex/bibliography/` on 2026-06-12; `style=apa` renders correct APA in-text (Chapter 1 verified, **0 undefined**). Self-repository citations removed (user decision): 19 `project_*_2026` keys deleted from both `.bib` files, `references.md`, and all chapters → **57 bib entries / 54 used**. Remaining: re-verify no undefined/unused after Chapters 2–6 are converted. |
