...
| T-092 | Phase 1 bootstrap LaTeX skeleton under `docs/thesis-latex/` (main.tex, config/, frontmatter/, 6 chapter placeholders, appendices/, bibliography/, figures/, scripts/) and migration harness (`thesis-latex-migration` skill + LaTeX architect agent + `latex-migration/` control docs). Fix the sample's `frontmatter.tex` page-numbering bug. No content conversion. | DONE | P0 | Codex | `docs/thesis-latex/` skeleton and harness files created; `pdflatex` stub compile passed; `latexmk` now works through `scripts/build.ps1` Perl autodetect. |
| T-093 | Phase 2 convert chapters 1–6 from approved Markdown to LaTeX, one chapter per approval gate (compile + content-equivalence + visual review before next chapter). | DONE | P0 | Codex | Chapters 1-6 converted, reviewed, and user-approved by 2026-06-13. Latest approved Chapter 6 build before front-matter conversion: `build/main.pdf` 71 pages, 0 undefined citations/references; marker scan clean. |
| T-094 | Phase 2 convert front matter: cover, approval/declaration, acknowledgements, abstract, abbreviations, definitions, author contribution — using the author/supervisor metadata from `README.md`, not the sample's personal data. | COMPLETE | P0 | Codex | Codex converted the five content stubs; Claude reviewed PASS (`reviews/frontmatter-latex-review-2026-06-14.md`, added XML/URL to abbreviations); **user-approved 2026-06-14**. `cover.tex`/`approval.tex` unchanged. Build PASS, `build/main.pdf` 74 pages, 0 undefined, 0 overfull. |
| T-095 | Phase 2 convert appendices A-E to LaTeX. | DONE | P1 | Codex | Codex converted Appendices A-E into the grouped `\appendixitem{X}{Title}` structure, removed all `\migrationpending` markers, preserved Appendix D execution evidence (18 tests, 18 passed, 0 failed, 0 skipped, 28.890057s), and scoped Appendix C/E automation-skill references to the four safe automation skills only. Claude review PASS + two layout feedbacks applied (continuous `\appendixitem` flow; Appendix B B.1 compacted; Appendix C merged into one `longtable`) + path/file column de-risked. Build PASS: `build/main.pdf` 78 pages; 0 undefined; 0 overfull; marker/safety scan clean. Review `reviews/appendices-latex-review-2026-06-14.md`. **User-approved 2026-06-14.** |
| T-096 | Bibliography migration: port `references.bib`, set `biblatex style=apa` (APA 7 confirmed by user 2026-06-12), verify every `[@key]`/`\cite{}` resolves and no unused/undefined entries remain. | DONE | P0 | Claude/Codex | `references.bib` ported into `docs/thesis-latex/bibliography/` on 2026-06-12; `style=apa` renders correct APA in-text. Self-repository (`project_*`) and self-artifact (`execution_*`/`visual_*`) citations removed (user decisions); doc entries converted `@misc`→`@online` for correct APA 7 URL/retrieval-date rendering. **Final verification PASS (Claude, 2026-06-14):** 52 entries, 49 cited (all resolve), 0 undefined, 0 biber warnings, 0 duplicate keys, no self-repo/self-artifact keys, no `[Accessed]`/missing-URL/`n.d.`; APA 7 rendering confirmed. Three vetted-but-uncited entries left as a harmless candidate bank (do not print). Review `reviews/bibliography-final-verification-2026-06-14.md`. |
| T-097 | Figure migration: convert Mermaid SVG masters to PDF vector for LaTeX; keep screenshots as PNG; update figure paths. | DONE | P1 | Codex | Regenerated 13 active Mermaid PDF vector assets with `scripts/convert-svg-to-pdf.ps1`; screenshots/report figures remain PNG. Updated `figure-register.md` to match final LaTeX numbering and the removed redundant shared-runtime figure. Build PASS: `build/main.pdf` 78 pages, 0 undefined citations/references; no `.svg` graphics are included by LaTeX. Review note: `reviews/figure-migration-review-2026-06-14.md`. **Claude review PASS + user-approved 2026-06-14:** verified all 21 `\includegraphics` targets exist (13 valid `%PDF` + 8 PNG), 0 undefined, 0 overfull, LoF = 20 figures continuous (3.1–3.12, 4.1–4.4, 5.1–5.4) matching the includes; orphan unused `shared-runtime-context-flow.pdf` deleted. |
| T-098 | Final LaTeX/Overleaf readiness QA gate: clean `latexmk` compile, VNUK front-matter order, numbering, cross-references, citation keys, margins/typography, content parity, no leftover PLACEHOLDER/NEEDS_INPUT. | DONE | P0 | Claude | **QA gate PASS (Claude, 2026-06-14).** Forced clean rebuild (`latexmk -gg -xelatex`) EXIT 0, **78 pages**, 0 undefined, 0 overfull `\hbox`, 0 underfull `\vbox`, 0 missing chars, 0 unresolved `??`, 0 non-biblatex warnings, 0 hyperref duplicate destinations, no content markers. LoF 20 figures continuous + all assets exist; LoT 13 tables; front-matter order matches approved T-094; captions bottom; APA 7 bib verified. One open item — left/binding margin — **resolved by user 2026-06-14: keep all margins 2.0 cm** (intentional deviation from the guideline's 3.0 cm, recorded in spec/checklist/latex-progress/document-setup-summary). Review: `reviews/latex-readiness-review.md`. |
| T-099 | Overleaf Premium Git sync: create dedicated Overleaf project, clone as separate folder, push the LaTeX project, and document the pull/compile/push protocol. | DONE | P1 | User + Claude | **DONE 2026-06-14 via GitHub Sync route** (user used Overleaf's GitHub Sync, not direct git.overleaf.com). User created Overleaf project (id `6a2ebaad5239feb373495b8e`) linked to GitHub `LeTiepTuyen/Thesis---UI-and-API-Automation-Framework-Playwright-`. Claude made the repo **private** (was public — protects thesis + signature), cloned to `D:\DevTools\Projects\thesis-overleaf-sync`, replaced the Overleaf import stub with the full project (48 files: main.tex `report` class + config/frontmatter/chapters/appendices/bibliography/figures + Overleaf-safe `latexmkrc`/`.gitignore`), committed `8ebeef8`, pushed to `main`. User pulled into Overleaf and **compiled successfully: 77 pages, XeLaTeX, Calibri→Carlito fallback (expected)**. Security: advised user to revoke the Overleaf Git token they exposed in chat (not needed with GitHub Sync). Guide: `latex-migration/overleaf-sync-guide.md`. |
| T-100 | Apply the user-provided thesis formatting guideline to the LaTeX project and shorten the thesis title across related project/thesis files. | DONE | P0 | Codex + Claude | Title shortened; LaTeX config updated to XeLaTeX, Calibri, A4, single-sided binding margin 3 cm/others 2 cm, left-aligned, 1.5 spacing, 14pt bold chapter headings, bottom captions, APA references. Claude follow-up (user-approved 2026-06-12): font raised to **12pt**; paragraph spacing + orphan/widow control added; blank page after cover removed; VNUK template alignment (title page copyright + degree statement, signature page removed, approval-page wording, "Author Contribution Statement"/"Reference List" renames); TOC titled "Table of Contents" with dotted leaders, List of Figures/Tables entries, and chapter-only bold. See `latex-migration/compile-issue-log.md`. |
