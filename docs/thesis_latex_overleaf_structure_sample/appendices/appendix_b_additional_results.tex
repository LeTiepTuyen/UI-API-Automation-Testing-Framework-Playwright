...
\midrule
\texttt{autocorr\_fixed\_val\_acc\_69\_6} & head-Y oscillation & landmarks & CNN+LSTM / historical & 69.6\% \\
\addlinespace
\texttt{autocorr\_fixed\_val\_acc\_72\_9} & \texttt{contact\_diff} & landmarks & CNN+LSTM / historical & 72.9\% \\
\addlinespace
\texttt{autocorr\_fixed\_biLSTM} & \texttt{contact\_diff} & landmarks + velocity & BiLSTM & Not recorded \\
\addlinespace
\texttt{autocorr\_fixed\_val\_acc\_76\_5} & \texttt{contact\_diff} + autocorr period & RGB crop $224 \times 224$ & MobileNetV2 + LSTM & 76.5\% \\
\addlinespace
\texttt{02\_baseline\_landmark} & \texttt{contact\_diff} & 13 landmarks + velocity & best of MLP, 1D-CNN, BiLSTM, CNN-BiLSTM & Not applicable \\
\bottomrule
\end{tabularx}
\end{table}
...
Chapter~\ref{ch:results}. The controlled \texttt{02\_baseline\_landmark}
result is reported in Chapter~\ref{ch:results} using test accuracy and
macro-F1 in the $[0,1]$ scale, not as a historical validation percentage.
