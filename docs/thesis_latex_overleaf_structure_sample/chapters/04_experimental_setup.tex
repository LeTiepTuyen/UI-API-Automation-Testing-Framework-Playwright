...
% appear here; they are reported in Chapter 5. Training hyper-parameters that
% are not fixed in this document are left as \texttt{[INSERT VALUE]} rather than
% invented. Architecture parameter counts are configuration facts and are
% stated where known.
% =====================================================================
...
\subsection{Handmade Landmark Upper Bound}
\label{sec:setup-handmade}

The handmade pipeline uses manually segmented cycles and the landmark
representation. It serves as the \emph{upper bound}: it estimates the achievable
classification performance when cycle segmentation is reliable, isolating
classification difficulty from automatic-detection difficulty. It is
explicitly not a deployable baseline.

\subsection{Autocorr Landmark Baseline}
\label{sec:setup-autocorr}

The autocorrelation landmark pipeline is the main \emph{automatic baseline}. It
detects cycles from the \texttt{contact\_diff} signal using the
autocorrelation-based detector of Section~\ref{sec:method-autocorr} and uses the
same landmark representation and classifiers as the handmade pipeline, so that
the two differ only in cycle source.

\subsection{Autocorr Image-Based Pipeline}
\label{sec:setup-image}

The image-based pipeline reuses the \emph{identical} autocorrelation cycle
source as the landmark baseline but represents each phase as a runner-centred
RGB crop classified by a MobileNetV2--LSTM model
(Section~\ref{sec:method-image-rep}). Holding the cycle source fixed makes this
a controlled comparison of \emph{representation}, separate from any historical
image-based development milestone.

\subsection{Boundary Detector Pipeline}
\label{sec:setup-bd}

The boundary-detector pipeline replaces rule-based autocorrelation with a
learned BiLSTM foot-strike detector (Section~\ref{sec:method-bd}); downstream
stages are unchanged. It is reported as a supplementary pipeline.

\subsection{Raw Sequence Pipeline}
\label{sec:setup-raw}

The raw-sequence pipeline omits cycle cutting entirely and classifies the whole
video's pose sequence with a recurrent model (GRU/BiLSTM,
Section~\ref{sec:method-gru}). Here one sample corresponds to one video, so its
evaluation unit differs from the cycle-based pipelines; it is treated as an
exploratory, supplementary configuration and is not part of the controlled
...
operate on eight-phase landmark inputs (Section~\ref{sec:method-phases}). The
image model uses eight RGB crops per detected cycle and a MobileNetV2--LSTM
architecture \cite{sandler2018mobilenetv2, donahue2015lrcn}. Table~\ref{tab:model-config}
reports only configuration information that is supported by the provided thesis
files, experiment inventory, and exported result reports. Optimiser, learning
rate, batch size, epoch count, and loss function were not exported in the
available final reports; therefore, these values are not inferred here.

\begin{table}[htbp]
\centering
\caption{Model configuration evidence available from the provided experiment files. Parameter counts are reported only when they are explicitly available in the exported model-ablation report. Missing training hyper-parameters are marked as not exported rather than inferred.}
\label{tab:model-config}
\renewcommand{\arraystretch}{1.3} % Tăng khoảng cách dòng một chút cho dễ đọc
\small % Giảm cỡ chữ xuống một chút để bảng gọn gàng hơn

\begin{tabularx}{\textwidth}{lXXXX} % Chiếm đúng bằng chiều rộng văn bản
\toprule
Model & Pipeline use & Input representation & Parameters & Training hyper-parameters \\
\midrule
MLP & Handmade and autocorr landmark model ablation & Eight-phase landmark sequence with selected landmarks and velocity features & $193{,}025$ & Not exported in provided reports \\
\addlinespace
1D-CNN & Handmade and autocorr landmark model ablation & Eight-phase landmark sequence with selected landmarks and velocity features & $40{,}257$ & Not exported in provided reports \\
\addlinespace
BiLSTM & Handmade and autocorr landmark model ablation & Eight-phase landmark sequence with selected landmarks and velocity features & $173{,}441$ & Not exported in provided reports \\
\addlinespace
CNN-BiLSTM & Handmade and autocorr landmark model ablation & Eight-phase landmark sequence with selected landmarks and velocity features & $82{,}113$ & Not exported in provided reports \\
\addlinespace
MobileNetV2--LSTM & Autocorr image-based representation experiment & Eight-phase RGB runner crops, resized to $224\times224$ & Not exported & Not exported in provided reports \\
\addlinespace
GRU / raw sequence model & Supplementary raw-sequence experiment & Video-level raw landmark sequence representation & Not exported & Not exported in provided reports \\
\bottomrule
\end{tabularx}
\end{table}

The four landmark-sequence models are retained in the main configuration table
because their parameter counts are present in the exported model-ablation report.
The MobileNetV2--LSTM and raw-sequence GRU rows are retained for completeness
because they correspond to supplementary experiments described in the experiment
inventory, but their exact parameter counts and training hyper-parameters were
not available in the exported evidence reviewed for this thesis. This avoids
reporting unverified implementation details while keeping the experimental scope
transparent.
...
Let $\mathrm{TP}$, $\mathrm{FP}$, $\mathrm{FN}$, and $\mathrm{TN}$ denote the
counts of true positives, false positives, false negatives, and true negatives
for a given class. The following metrics are used.

\emph{Accuracy} is the fraction of correctly classified samples:
\begin{equation}
\mathrm{Accuracy} =
\frac{\mathrm{TP} + \mathrm{TN}}{\mathrm{TP} + \mathrm{TN} + \mathrm{FP} + \mathrm{FN}}.
\end{equation}

\emph{Precision} and \emph{recall} for a class are
\begin{equation}
\mathrm{Precision} = \frac{\mathrm{TP}}{\mathrm{TP} + \mathrm{FP}},
\qquad
\mathrm{Recall} = \frac{\mathrm{TP}}{\mathrm{TP} + \mathrm{FN}},
\end{equation}
measuring, respectively, the proportion of positive predictions that are correct
and the proportion of actual positives that are recovered.

The \emph{F1 score} is their harmonic mean:
\begin{equation}
\mathrm{F1} =
...
The \emph{macro-averaged F1} (macro-F1) is the unweighted mean of the per-class
F1 scores over the two classes (correct, false):
\begin{equation}
\mathrm{macro\text{-}F1} =
